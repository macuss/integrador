...
public class PDFResumen {
    ...
    public static void crear(String ruta, int total, int passed, int failed, float passRate, float failRate,
            float timeAvg, float timeTotal, int slowest, int fastest, List<Integer> filasErrores) {
        try {
            ...
            table.setWidthPercentage(80); // Ancho de la tabla (80% de la página)
            table.setWidths(new float[] { 3f, 2f }); // Proporción de ancho de las columnas (30% para etiqueta, 20% para valor)
            table.setSpacingBefore(10);
            table.setSpacingAfter(10);
            
      
            addTableRow(table, "Total de Pruebas Ejecutadas:", String.valueOf(total), keyFont, valueFont);
            addTableRow(table, "Total de PASS:", String.valueOf(passed), keyFont, new Font(valueFont.getFamily(), 10, Font.BOLD, BaseColor.GREEN));
            addTableRow(table, "Total de FAIL:", String.valueOf(failed), keyFont, new Font(valueFont.getFamily(), 10, Font.BOLD, BaseColor.RED));
            ...
            addTableRow(timeTable, "Tiempo Total de Ejecución:", String.format("%.2f ms", timeTotal), keyFont, valueFont);
            addTableRow(timeTable, "Tiempo Promedio por Prueba:", String.format("%.2f ms", timeAvg), keyFont, valueFont);
            addTableRow(timeTable, "Prueba Más Lenta:", String.valueOf(slowest) + " ms", keyFont, new Font(valueFont.getFamily(), 10, Font.ITALIC));
            addTableRow(timeTable, "Prueba Más Rápida:", String.valueOf(fastest) + " ms", keyFont, new Font(valueFont.getFamily(), 10, Font.ITALIC));
            ...
            doc.close();
            System.out.println("PDF de resumen creado exitosamente en: " + ruta);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
